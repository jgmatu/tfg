...
public:
	...
	void cloud_cb(const sensor_msgs::PointCloud2::ConstPtr& cloud_in)
	{
		...
		permanents_.publish(cloud_pub_, cloud_in->header.frame_id);
	...
	{
		if(instants_.empty()) {
			// No plobs detected in this callback
			// from CNN.
		...
		if(permanents_.empty()) {
			// Permanents is empty, storage
			// all instants plobs detected
			// by the CNN.
		...
		// and add or update plobs of one category we have already added.
	...
	void
	insert_by_features(std::string category)
	{
		...
		permanents_.update(results["update"]);
	...
	void set_octomaps_publishers()
	{
		...
		for (it = categories.begin(); it != categories.end(); it++) {
			...
			sprintf(topic_id, "/octomap/%s", it->c_str());
			octo_pubs_.push_back(nh_.advertise<octomap_msgs::Octomap>(topic_id, 0, true));
		}
	}
...
private:
	...
	std::vector<ros::Publisher> octo_pubs_;
